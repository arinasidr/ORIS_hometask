...
def register():
    if request.method == 'POST':
        ...
        if not username or not password:
            return render_template('register.html', error="нужно указать имя пользователя и пароль")
...
def login():
    if request.method == 'POST':
        ...
        password = request.form.get('password', '')
        if not username or not password:
            return render_template('login.html', error='нужно указать имя пользователя и пароль')
...
def add_to_cart(product_id):
    init_cart()
    cart = session['cart']
    cart[str(product_id)] = cart.get(str(product_id), 0) + 1 #запись товара в корзину, котрая храниться в сесии
    session.modified = True #строка заставляет фласк сохранить изменения
